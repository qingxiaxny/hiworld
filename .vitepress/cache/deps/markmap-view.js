...
function blurh(blur3, T2, S, w, h) {
  for (let y3 = 0, n = w * h; y3 < n; ) {
    blur3(T2, S, y3, y3 += w, 1);
  }
}
function blurv(blur3, T2, S, w, h) {
  for (let x3 = 0, n = w * h; x3 < w; ++x3) {
    blur3(T2, S, x3, x3 + n, w);
  }
}
function blurfImage(radius) {
  const blur3 = blurf(radius);
  return (T2, S, start2, stop, step) => {
    start2 <<= 2, stop <<= 2, step <<= 2;
    blur3(T2, S, start2 + 0, stop + 0, step);
    blur3(T2, S, start2 + 1, stop + 1, step);
    blur3(T2, S, start2 + 2, stop + 2, step);
    blur3(T2, S, start2 + 3, stop + 3, step);
  };
}
function blurf(radius) {
  ...
  return (T2, S, start2, stop, step) => {
    ...
    for (let i = start2, j2 = stop; i <= j2; i += step) {
      ...
      T2[i] = (sum4 + t * (S[Math.max(start2, i - s1)] + S[Math.min(stop, i + s1)])) / w;
...
var Adder = class {
  ...
  add(x3) {
    ...
    for (let j2 = 0; j2 < this._n && j2 < 32; j2++) {
      const y3 = p[j2], hi = x3 + y3, lo = Math.abs(x3) < Math.abs(y3) ? x3 - (hi - y3) : y3 - (hi - x3);
      if (lo) p[i++] = lo;
      x3 = hi;
    }
    p[i] = x3;
...
Dispatch.prototype = dispatch.prototype = {
  ...
  copy: function() {
    var copy3 = {}, _2 = this._;
    for (var t in _2) copy3[t] = _2[t].slice();
...
function basis_default(values) {
  ...
  return function(t) {
    var i = t <= 0 ? t = 0 : t >= 1 ? (t = 1, n - 1) : Math.floor(t * n), v1 = values[i], v22 = values[i + 1], v0 = i > 0 ? values[i - 1] : 2 * v1 - v22, v3 = i < n - 1 ? values[i + 2] : 2 * v22 - v1;
...
function basisClosed_default(values) {
  var n = values.length;
  return function(t) {
    var i = Math.floor(((t %= 1) < 0 ? ++t : t) * n), v0 = values[(i + n - 1) % n], v1 = values[i % n], v22 = values[(i + 1) % n], v3 = values[(i + 2) % n];
    return basis((t - i / n) * n, v0, v1, v22, v3);
  };
}
...
function string_default(a2, b) {
  ...
  return s2.length < 2 ? q2[0] ? one(q2[0].x) : zero2(b) : (b = q2.length, function(t) {
...
function find_default(x3, y3, radius) {
  var data, x02 = this._x0, y0 = this._y0, x12, y1, x22, y22, x32 = this._x1, y32 = this._y1, quads = [], node = this._root, q2, i;
  ...
  while (q2 = quads.pop()) {
    if (!(node = q2.node) || (x12 = q2.x0) > x32 || (y1 = q2.y0) > y32 || (x22 = q2.x1) < x02 || (y22 = q2.y1) < y0) continue;
    if (node.length) {
...
function remove_default3(d) {
  ...
  var parent, node = this._root, retainer, previous, next, x02 = this._x0, y0 = this._y0, x12 = this._x1, y1 = this._y1, x3, y3, xm, ym, right, bottom, i, j2;
  if (!node) return this;
...
function polygonContains_default(polygon, point6) {
  ...
  for (var i = 0, n = polygon.length; i < n; ++i) {
    ...
    for (var j2 = 0; j2 < m; ++j2, lambda0 = lambda1, sinPhi0 = sinPhi1, cosPhi0 = cosPhi1, point0 = point1) {
      var point1 = ring[j2], lambda1 = longitude(point1), phi1 = point1[1] / 2 + quarterPi, sinPhi1 = sin2(phi1), cosPhi1 = cos2(phi1), delta = lambda1 - lambda0, sign3 = delta >= 0 ? 1 : -1, absDelta = sign3 * delta, antimeridian = absDelta > pi4, k2 = sinPhi0 * sinPhi1;
...
var gamma_default = function sourceRandomGamma(source) {
  ...
  function randomGamma(k2, theta) {
    ...
    return function() {
      do {
        do {
          var x3 = randomNormal(), v3 = 1 + c3 * x3;
        } while (v3 <= 0);
        v3 *= v3 * v3;
        var u4 = 1 - source();
      } while (u4 >= 1 - 0.0331 * x3 * x3 * x3 * x3 && Math.log(u4) >= 0.5 * x3 * x3 + d * (1 - v3 + Math.log(v3)));
...
function formatLocale(locale3) {
  ...
  function newFormat(specifier, formats2) {
    return function(date) {
      var string = [], i = -1, j2 = 0, n = specifier.length, c3, pad3, format2;
...
function link2(curve) {
  let source = linkSource, target = linkTarget, x3 = x2, y3 = y2, context = null, output = null, path2 = withPath(link3);
  function link3() {
    let buffer;
  ...
  link3.x = function(_2) {
    return arguments.length ? (x3 = typeof _2 === "function" ? _2 : constant_default10(+_2), link3) : x3;
  };
  link3.y = function(_2) {
    return arguments.length ? (y3 = typeof _2 === "function" ? _2 : constant_default10(+_2), link3) : y3;
  };
...
Bundle.prototype = {
  ...
  lineEnd: function() {
    ...
    if (j2 > 0) {
      var x02 = x3[0], y0 = y3[0], dx = x3[j2] - x02, dy = y3[j2] - y0, i = -1, t;
      while (++i <= j2) {
        t = i / j2;
        this._basis.point(
...
function zoom_default2() {
  ...
  zoom.wheelDelta = function(_2) {
    return arguments.length ? (wheelDelta = typeof _2 === "function" ? _2 : constant_default11(+_2), zoom) : wheelDelta;
  };
  zoom.filter = function(_2) {
    return arguments.length ? (filter3 = typeof _2 === "function" ? _2 : constant_default11(!!_2), zoom) : filter3;
  };
  zoom.touchable = function(_2) {
    return arguments.length ? (touchable = typeof _2 === "function" ? _2 : constant_default11(!!_2), zoom) : touchable;
  };
  zoom.extent = function(_2) {
    return arguments.length ? (extent2 = typeof _2 === "function" ? _2 : constant_default11([[+_2[0][0], +_2[0][1]], [+_2[1][0], +_2[1][1]]]), zoom) : extent2;
  };
...
function oe(e) {
  const t = {}, i = { ...e }, { color: n, colorFreezeLevel: s2, lineWidth: o } = i;
  ...
  if (s2) {
    ...
    t.color = (l) => (l = {
      ...
      state: {
        ...l.state,
        path: l.state.path.split(".").slice(0, s2).join(".")
      }
    }, c3(l));
  }
  if (o) {
...
function St(e) {
  var t = this, i, n = [t], s2, o, h;
  do
    for (i = n.reverse(), n = []; t = i.pop(); )
      if (e(t), s2 = t.children, s2) for (o = 0, h = s2.length; o < h; ++o)
        n.push(s2[o]);
  while (n.length);
  return this;
}
function wt(e) {
  for (var t = this, i = [t], n, s2; t = i.pop(); )
    if (e(t), n = t.children, n) for (s2 = n.length - 1; s2 >= 0; --s2)
      i.push(n[s2]);
  return this;
}
function Rt(e) {
  for (var t = this, i = [t], n = [], s2, o, h; t = i.pop(); )
    if (n.push(t), s2 = t.children, s2) for (o = 0, h = s2.length; o < h; ++o)
      i.push(s2[o]);
  for (; t = n.pop(); )
    e(t);
  return this;
...
function Xt(e) {
  return this.eachAfter(function(t) {
    for (var i = +e(t.data) || 0, n = t.children, s2 = n && n.length; --s2 >= 0; ) i += n[s2].value;
    t.value = i;
  });
}
function _t(e) {
...
function Ot(e) {
  for (var t = this, i = At(t, e), n = [t]; t !== i; )
    t = t.parent, n.push(t);
  for (var s2 = n.length; e !== i; )
    n.splice(s2, 0, e), e = e.parent;
  return n;
}
function At(e, t) {
  if (e === t) return e;
  var i = e.ancestors(), n = t.ancestors(), s2 = null;
  for (e = i.pop(), t = n.pop(); e === t; )
    s2 = e, e = i.pop(), t = n.pop();
  return s2;
}
function Mt() {
...
function j(e, t) {
  var i = new _(e), n = +e.value && (i.value = e.value), s2, o = [i], h, a2, c3, l;
  for (t == null && (t = Ft); s2 = o.pop(); )
    if (n && (s2.value = +s2.data.value), (a2 = t(s2.data)) && (l = a2.length))
      for (s2.children = new Array(l), c3 = l - 1; c3 >= 0; --c3)
        o.push(h = s2.children[c3] = new _(a2[c3])), h.parent = s2, h.depth = s2.depth + 1;
...
function tt(e) {
  ...
  function n(a2) {
    const c3 = h(o(), a2, (l) => l.children);
    return c3.update(), c3.data;
  }
  function s2() {
    const a2 = i("nodeSize"), c3 = i("spacing");
  ...
  function o() {
    const a2 = s2(), c3 = i("nodeSize"), l = i("spacing");
  ...
  function h(a2, c3, l) {
    const d = (p, m) => {
      ...
      const x3 = l(p) || [];
      return u4.children = x3.length === 0 ? null : x3.map((y3) => d(y3, u4)), u4.children && Object.assign(u4, u4.children.reduce(
        (y3, k2) => ({
          height: Math.max(y3.height, k2.height + 1),
          length: y3.length + k2.length
        }),
        u4
      )), u4;
    };
  ...
  return Object.assign(n, {
    nodeSize(a2) {
      return arguments.length ? (t.nodeSize = a2, n) : t.nodeSize;
    },
    spacing(a2) {
      return arguments.length ? (t.spacing = a2, n) : t.spacing;
    },
    children(a2) {
      return arguments.length ? (t.children = a2, n) : t.children;
    },
    hierarchy(a2, c3) {
    ...
    dump(a2) {
      const c3 = i("nodeSize"), l = (d) => (p) => {
        const m = d + "  ", u4 = d + "    ", { x: x3, y: y3 } = p, k2 = c3(p), b = p.children || [], z = b.length === 0 ? " " : `,${m}children: [${u4}${b.map(l(u4)).join(u4)}${m}],${d}`;
        return `{ size: [${k2.join(", ")}],${m}x: ${x3}, y: ${y3}${z}},`;
      };
      return l(`
`)(a2);
    }
...
var et = (e, t = 0) => (e.y = t, (e.children || []).reduce((i, n) => {
  const [s2, o] = i;
  et(n, e.y + e.ySize);
  const h = (s2 === 0 ? n.lExt : n.rExt).bottom;
  s2 !== 0 && Kt(e, s2, o);
  const a2 = Ut(h, s2, o);
  return [s2 + 1, a2];
}, [0, null]), It(e), Qt(e), e);
var rt = (e, t, i) => {
  ...
  const n = t + e.relX;
  return e.relX = n + e.prelim - i, e.prelim = 0, e.x = i + e.relX, (e.children || []).forEach((s2) => rt(s2, n, e.x)), e;
};
var It = (e) => {
  (e.children || []).reduce((t, i) => {
    const [n, s2] = t, o = n + i.shift, h = s2 + o + i.change;
    return i.relX += h, [o, h];
  }, [0, 0]);
};
var Kt = (e, t, i) => {
  const n = e.children[t - 1], s2 = e.children[t];
  let o = n, h = n.relX, a2 = s2, c3 = s2.relX, l = true;
  for (; o && a2; ) {
    o.bottom > i.lowY && (i = i.next);
    const d = h + o.prelim - (c3 + a2.prelim) + o.xSize / 2 + a2.xSize / 2 + o.spacing(a2);
    (d > 0 || d < 0 && l) && (c3 += d, Gt(s2, d), Vt(e, t, i.index, d)), l = false;
    const p = o.bottom, m = a2.bottom;
    p <= m && (o = Zt(o), o && (h += o.relX)), p >= m && (a2 = Yt(a2), a2 && (c3 += a2.relX));
  }
  !o && a2 ? qt(e, t, a2, c3) : o && !a2 && Jt(e, t, o, h);
};
var Gt = (e, t) => {
  e.relX += t, e.lExtRelX += t, e.rExtRelX += t;
};
var Vt = (e, t, i, n) => {
  const s2 = e.children[t], o = t - i;
  if (o > 1) {
    const h = n / o;
    e.children[i + 1].shift += h, s2.shift -= h, s2.change -= n - h;
  }
};
var Yt = (e) => e.hasChildren ? e.firstChild : e.lThr;
var Zt = (e) => e.hasChildren ? e.lastChild : e.rThr;
var qt = (e, t, i, n) => {
  const s2 = e.firstChild, o = s2.lExt, h = e.children[t];
  o.lThr = i;
  const a2 = n - i.relX - s2.lExtRelX;
  o.relX += a2, o.prelim -= a2, s2.lExt = h.lExt, s2.lExtRelX = h.lExtRelX;
};
var Jt = (e, t, i, n) => {
  const s2 = e.children[t], o = s2.rExt, h = e.children[t - 1];
  o.rThr = i;
  const a2 = n - i.relX - s2.rExtRelX;
  o.relX += a2, o.prelim -= a2, s2.rExt = h.rExt, s2.rExtRelX = h.rExtRelX;
};
var Qt = (e) => {
  if (e.hasChildren) {
...
var it = class _it {
  constructor(t, i) {
    this.options = { ...Q }, this._disposeList = [], this.handleZoom = (n) => {
      const { transform: s2 } = n;
      this.g.attr("transform", s2);
    }, this.handlePan = (n) => {
      n.preventDefault();
      const s2 = transform(this.svg.node()), o = s2.translate(
        -n.deltaX / s2.k,
        -n.deltaY / s2.k
      );
      this.svg.call(this.zoom.transform, o);
    }, this.handleClick = (n, s2) => {
      let o = this.options.toggleRecursively;
      (q ? n.metaKey : n.ctrlKey) && (o = !o), this.toggleNode(s2, o);
...
`);
  ...
  async toggleNode(t, i = false) {
    var s2, o;
    const n = (s2 = t.payload) != null && s2.fold ? 0 : 1;
    i ? walkTree(t, (h, a2) => {
      h.payload = {
        ...h.payload,
        fold: n
  ...
  _initializeData(t) {
    let i = 0;
    const { color: n, initialExpandLevel: s2 } = this.options;
    let o = 0, h = 0;
    return walkTree(t, (a2, c3, l) => {
      var p, m, u4, x3;
      h += 1, a2.children = (p = a2.children) == null ? void 0 : p.map((y3) => ({ ...y3 })), i += 1, a2.state = {
        ...a2.state,
        depth: h,
        id: i,
        rect: {
        ...
        size: [0, 0]
      }, a2.state.key = [(m = l == null ? void 0 : l.state) == null ? void 0 : m.id, a2.state.id].filter(Boolean).join(".") + Et(a2.content), a2.state.path = [(u4 = l == null ? void 0 : l.state) == null ? void 0 : u4.path, a2.state.id].filter(Boolean).join("."), n(a2);
      const d = ((x3 = a2.payload) == null ? void 0 : x3.fold) === 2;
      d ? o += 1 : (o || s2 >= 0 && a2.state.depth >= s2) && (a2.payload = { ...a2.payload, fold: 1 }), c3(), d && (o -= 1), h -= 1;
    }), t;
  }
  _relayout() {
    ...
    const { lineWidth: t, paddingX: i, spacingHorizontal: n, spacingVertical: s2 } = this.options, o = tt({}).children((c3) => {
      var l;
      if (!((l = c3.payload) != null && l.fold)) return c3.children;
    }).nodeSize((c3) => {
      const [l, d] = c3.data.state.size;
      return [d, l + (l ? i * 2 : 0) + n];
    }).spacing((c3, l) => (c3.parent === l.parent ? s2 : s2 * 2) + t(c3.data)), h = o.hierarchy(this.state.data);
    o(h);
    const a2 = h.descendants();
    a2.forEach((c3) => {
      const l = c3.data;
      l.state.rect = {
        x: c3.y,
      ...
      x1: min(a2, (c3) => c3.data.state.rect.x) || 0,
      y1: min(a2, (c3) => c3.data.state.rect.y) || 0,
      x2: max(
        a2,
  ...
  _getHighlightRect(t) {
    const i = this.svg.node(), s2 = 4 / transform(i).k, o = {
      ...t.state.rect
    };
    return o.x -= s2, o.y -= s2, o.width += 2 * s2, o.height += 2 * s2, o;
  }
  async renderData(t) {
    const { paddingX: i, autoFit: n, color: s2, maxWidth: o, lineWidth: h } = this.options, a2 = this.state.data;
    if (!a2) return;
    const c3 = {}, l = {}, d = [];
    walkTree(a2, (r, g, f) => {
      var E2;
      (E2 = r.payload) != null && E2.fold || g(), c3[r.state.id] = r, f && (l[r.state.id] = f.state.id), d.push(r);
    });
    const p = {}, m = {}, u4 = (r) => {
      !r || p[r.state.id] || walkTree(r, (g, f) => {
        p[g.state.id] = r.state.id, f();
      });
    }, x3 = (r) => m[p[r.state.id]] || a2.state.rect, y3 = (r) => (c3[p[r.state.id]] || a2).state.rect;
    m[a2.state.id] = a2.state.rect, t && u4(t);
    let { highlight: k2 } = this.state;
    k2 && !c3[k2.state.id] && (k2 = void 0);
    let b = this.g.selectAll(v2(ee)).selectAll(v2("rect")).data(k2 ? [this._getHighlightRect(k2)] : []).join("rect").attr("x", (r) => r.x).attr("y", (r) => r.y).attr("width", (r) => r.width).attr("height", (r) => r.height);
    const z = this.g.selectAll(v2(M2)).each((r) => {
      ...
      (r) => [r],
      (r) => r.state.key
    ), N = F.enter().append("line").attr("stroke", (r) => s2(r)).attr("stroke-width", 0), B3 = F.merge(N), L = w.selectAll(v2("circle")).data(
      (r) => {
        var g;
        return (g = r.children) != null && g.length ? [r] : [];
      },
      (r) => r.state.key
    ), D3 = L.enter().append("circle").attr("stroke-width", 0).attr("r", 0).on("click", (r, g) => this.handleClick(r, g)).on("mousedown", T).merge(L).attr("stroke", (r) => s2(r)).attr(
      "fill",
      (r) => {
        var g;
        return (g = r.payload) != null && g.fold && r.children ? s2(r) : "var(--markmap-circle-open-bg)";
      }
    ...
    const K2 = O.merge(P), st = d.flatMap(
      (r) => {
        var g;
        return (g = r.payload) != null && g.fold ? [] : r.children.map((f) => ({ source: r, target: f }));
      }
    ), A5 = this.g.selectAll(v2(te)).data(st, (r) => r.target.state.key), at = A5.exit(), ot = A5.enter().insert("path", "g").attr("class", "markmap-link").attr("data-depth", (r) => r.target.state.depth).attr("data-path", (r) => r.target.state.path).attr("d", (r) => {
      const g = x3(r.target), f = [
        g.x + g.width,
        g.y + g.height
      ];
      return H({ source: f, target: f });
    }).attr("stroke-width", 0).merge(A5);
    this.svg.style(
      "--markmap-max-width",
      o ? `${o}px` : null
    ), await new Promise(requestAnimationFrame), this._relayout(), b = b.data(k2 ? [this._getHighlightRect(k2)] : []).join("rect"), this.transition(b).attr("x", (r) => r.x).attr("y", (r) => r.y).attr("width", (r) => r.width).attr("height", (r) => r.height), $.attr("transform", (r) => {
      const g = x3(r);
      return `translate(${g.x + g.width - r.state.rect.width},${g.y + g.height - r.state.rect.height})`;
    }), this.transition(S).attr("transform", (r) => {
      const g = y3(r), f = g.x + g.width - r.state.rect.width, E2 = g.y + g.height - r.state.rect.height;
      return `translate(${f},${E2})`;
    ...
    this.transition(ct).attr("x1", (r) => r.state.rect.width).attr("stroke-width", 0), N.attr("x1", (r) => r.state.rect.width).attr("x2", (r) => r.state.rect.width), B3.attr("y1", (r) => r.state.rect.height + h(r) / 2).attr("y2", (r) => r.state.rect.height + h(r) / 2), this.transition(B3).attr("x1", -1).attr("x2", (r) => r.state.rect.width + 2).attr("stroke", (r) => s2(r)).attr("stroke-width", h);
    const ht = S.selectAll(
      v2("circle")
    );
    this.transition(ht).attr("r", 0).attr("stroke-width", 0), D3.attr("cx", (r) => r.state.rect.width).attr("cy", (r) => r.state.rect.height + h(r) / 2), this.transition(D3).attr("r", 6).attr("stroke-width", "1.5"), this.transition(I).style("opacity", 0), K2.attr("width", (r) => Math.max(0, r.state.rect.width - i * 2)).attr("height", (r) => r.state.rect.height), this.transition(K2).style("opacity", 1), this.transition(at).attr("d", (r) => {
      const g = y3(r.target), f = [
        g.x + g.width,
        g.y + g.height + h(r.target) / 2
      ];
      return H({ source: f, target: f });
    }).attr("stroke-width", 0).remove(), this.transition(ot).attr("stroke", (r) => s2(r.target)).attr("stroke-width", (r) => h(r.target)).attr("d", (r) => {
      const g = r.source, f = r.target, E2 = [
  ...
  async fit(t = this.options.maxInitialScale) {
    const i = this.svg.node(), { width: n, height: s2 } = i.getBoundingClientRect(), { fitRatio: o } = this.options, { x1: h, y1: a2, x2: c3, y2: l } = this.state.rect, d = c3 - h, p = l - a2, m = Math.min(
      n / d * o,
      s2 / p * o,
      t
    ), u4 = identity5.translate(
      (n - d * m) / 2 - h * m,
      (s2 - p * m) / 2 - a2 * m
    ).scale(m);
  ...
  findElement(t) {
    let i;
    return this.g.selectAll(v2(M2)).each(function(s2) {
      s2 === t && (i = {
        data: s2,
        g: this
      });
  ...
  async ensureVisible(t, i) {
    var k2;
    const n = (k2 = this.findElement(t)) == null ? void 0 : k2.data;
    if (!n) return;
    const s2 = this.svg.node(), o = s2.getBoundingClientRect(), h = transform(s2), [a2, c3] = [
      n.state.rect.x,
      ...
      bottom: 0,
      ...i
    }, m = [p.left - a2, o.width - p.right - c3], u4 = [p.top - l, o.height - p.bottom - d], x3 = m[0] * m[1] > 0 ? Y2(m, Math.abs) / h.k : 0, y3 = u4[0] * u4[1] > 0 ? Y2(u4, Math.abs) / h.k : 0;
    if (x3 || y3) {
      const b = h.translate(x3, y3);
      return this.transition(this.svg).call(this.zoom.transform, b).end().catch(noop);
    }
  }
  async centerNode(t, i) {
    var x3;
    const n = (x3 = this.findElement(t)) == null ? void 0 : x3.data;
    if (!n) return;
    const s2 = this.svg.node(), o = s2.getBoundingClientRect(), h = transform(s2), a2 = (n.state.rect.x + n.state.rect.width / 2) * h.k + h.x, c3 = (n.state.rect.y + n.state.rect.height / 2) * h.k + h.y, l = {
      left: 0,
      right: 0,
      top: 0,
      bottom: 0,
      ...i
    }, d = (l.left + o.width - l.right) / 2, p = (l.top + o.height - l.bottom) / 2, m = (d - a2) / h.k, u4 = (p - c3) / h.k;
    if (m || u4) {
      const y3 = h.translate(m, u4);
      return this.transition(this.svg).call(this.zoom.transform, y3).end().catch(noop);
    }
  }
  /**
   ...
   */
  async rescale(t) {
    const i = this.svg.node(), { width: n, height: s2 } = i.getBoundingClientRect(), o = n / 2, h = s2 / 2, a2 = transform(i), c3 = a2.translate(
      (o - a2.x) * (1 - t) / a2.k,
      (h - a2.y) * (1 - t) / a2.k
    ).scale(t);
  ...
  static create(t, i, n = null) {
    const s2 = new _it(t, i);
    return n && s2.setData(n).then(() => {
      s2.fit();
    }), s2;
  }
};
